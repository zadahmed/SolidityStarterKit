...
contract Voting{
    ...
    struct Ballot{
        ...
        function addVotes(address[] calldata _voters) external onlyAdmin(){
        ...
        function results(uint ballotId) view external returns(Choice[] memory ){
            require(block.timestamp >= ballots[ballotId].end , 'cannot see the ballot results before ballot end');
